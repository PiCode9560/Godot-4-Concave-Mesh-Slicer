...
# Converts a normal mesh to an ArrayMesh. Arraymeshes are easier to modify. 
...
# The cross-section material is positioned and rotated based on the Transform3D.
func slice_mesh(slice_transform: Transform3D, mesh: Mesh, cross_section_material: Material = null) -> Array:
	...
	for surface_idx in range(mesh.get_surface_count()):
		...
		for face in range(mdt.get_face_count()):
			...
			# Add vertices to the SurfaceTool for the first set of faces.
			...
			# Add vertices to the SurfaceTool for the second set of faces.
	...
	for polygon in sorted_verts:
		...
		for v in range(len(triangulate) / 3):
			...
			# Check the side of the triangle and add vertices accordingly.
...
func _set_holes(polygons, norm, slice_transform):
	# If there are less than or equal to 1 polygon, nothing to do, return the input polygons.
	if len(polygons) <= 1:
		return polygons
	...
	for i in range(len(polygons)):
		...
		# Check if the current polygon has vertices.
		if len(polygon) > 0:
			...
			for j in range(len(polygons)):
				if j != i:
					...
					# Iterate through the vertices of the other polygon.
					for k in range(len(polygon_check)):
						...
						# Find the closest point on the current polygon to the other polygon's edge.
					...
					# Check if the point is inside the other polygon.
			...
			# If the point is inside an intersecting polygon, proceed to connect them.
			if is_inside:
				...
				# Continuously connect the polygons until they are fully connected.
				while not connected:
					...
					# Iterate through vertices of the current polygon.
					for j in range(len(polygon)):
						var vert = polygon[j]

						# Iterate through vertices of the target polygon.
						for l in range(len(polygon_to)):
							...
							# Check if there is an intersection with any edge of any polygon.
							...
							if not is_intersect:
								# Determine the normal of the connecting plane.
								...
								# Determine the closest edge to connect.
								...
								# Connect the polygons based on edge closeness.
...
func _add_to_vert_slice(v1,v2):
	# Round vertex positions to four decimal places (0.0001 precision) to minimize
...
# Function to find the closest edge between two normals
...
# Check the side of a point relative to a plane. Returns 1 if on the same side as the normal and 
